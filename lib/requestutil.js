  ...
  if (withMultipart) {
    ...
    (function loop() {
      try {
        ...
        if (self.params[key] instanceof Buffer) {
          multipart.addBuffer(
            key,
            self.params[key],
            self.params[key].mimetype || 'image/jpeg',
            'medias.jpg' || 'media.jpg'
          );
          loop();
        }
        // FIXME DANGEROUS security issue!!!!!!!
        /*else if (self.params[key].charAt(0) === '@') {
